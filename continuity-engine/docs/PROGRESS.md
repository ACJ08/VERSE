...
Most bugs so far were found by **looking at demo output**, not by tests — the
tests were green each time. Single-entity, single-scene fixtures hid every one
of them. Prefer fixtures with two characters across several scenes.
